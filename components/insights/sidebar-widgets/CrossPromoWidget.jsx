      ...
      <section className="sidebar-widget" aria-labelledby="sidebar-from-labkit">
        ...
        {top.length > 0 && (
          <ul className="sidebar-widget-list">
            {top.map((c) => (
              <li key={c.title}>
                <Link href={c.href} className="sidebar-widget-cross-link">
                  {c.title}
                </Link>
              </li>
            ))}
        ...
        <Link href="/labkit" className="sidebar-widget-link">
          Explore all tools →
        </Link>
      </section>
    );
    ...
    <section className="sidebar-widget" aria-labelledby="sidebar-from-insights">
      ...
      {recent.length > 0 && (
        <ul className="sidebar-widget-list">
          {recent.map((p) => (
            <li key={p.slug}>
              <Link href={`/insights/${p.slug}`} className="sidebar-widget-cross-item">
      ...
      <Link href="/insights" className="sidebar-widget-link">
        Explore all writing →
      </Link>
    </section>
  );
